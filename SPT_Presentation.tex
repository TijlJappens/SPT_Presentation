...
{SPT classification:\\from entangled pair states to recent proofs}
...
\begin{frame}
	...
	\begin{center}
		\begin{tikzpicture}
		\node (0) [circle,fill,inner sep=3pt] at (0,0) {};
		\node (0 -1) [] at (0,-0.5) {$\psi_0$};
		\node (1) [circle,fill,inner sep=3pt] at (5,0) {};
		\node (1 -1) [] at (5,-0.5) {$\psi_1$};
		\draw [line width=2pt,-{Stealth[scale = 1.2]}] (0) -- (1);
		\node (arrow) [] at (2.5,-0.5) {$\psi_t=\psi_0\circ\gamma^K_{0;t}$};
		\end{tikzpicture}
	\end{center}
